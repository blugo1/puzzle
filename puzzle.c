...
void main(void)
{
	...
	while(1)
	{
		...
		pal_col(17,touch?i:0xFF);//set first sprite color
		...
		for(i=0;i<2;++i)
		{
			...
			if(pad&PAD_LEFT &&cat_x[i]>  0) cat_x[i]-=5;
			if(pad&PAD_RIGHT&&cat_x[i]<232) cat_x[i]+=5;
			if(pad&PAD_UP   &&cat_y[i]>  0) cat_y[i]-=5;
			if(pad&PAD_DOWN &&cat_y[i]<212) cat_y[i]+=5;
		}
